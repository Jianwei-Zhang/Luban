...
my $mainrulergridlinecolor = 'black';
my $gridlinecolor = 'silver';
my $maingridlinecolor = 'black';
my $graduationsize = 10;
my $graduationcolor = 'black';
...
$featurenamewrap->set_font('OCRAEXT', $featuresize); #1X feature height font size
...
for ( map { $_->[0] } sort { $a->[1] <=> $b->[1] || $a->[2] cmp $b->[2]} map { [$_, /(\d+)/, uc($_)]} (keys %seq)) #seqid was sorted by numbers, then letters
{
	...
	for (@{$seq{$seqid}})
	{
		...
		if($objectline[4] == 1) #plus strand
		{
			...
			{
				$mainlayer->filledRectangle($objectstart,$imageheader+$seqnumber*$unitseqheight+$tracksize-$goodlevel*($featuresize+1)-$featuresize,$objectend,$imageheader+$seqnumber*$unitseqheight+$tracksize-$goodlevel*($featuresize+1)-1,$mainlayercolor{$featurecolor{$objectline[1]}});		
			...
			if($featurenamelength)
			{
				...
				$featurenamewrap->draw($objectend,$imageheader+$seqnumber*$unitseqheight+$tracksize-$goodlevel*($featuresize+1)-$featuresize);
		...
		else # minus strand
		{
			...
			{
				$mainlayer->filledRectangle($objectstart,$imageheader+$seqnumber*$unitseqheight+$tracksize+$barheight+$goodlevel*($featuresize+1)+1,$objectend,$imageheader+$seqnumber*$unitseqheight+$tracksize+$barheight+$goodlevel*($featuresize+1)+$featuresize,$mainlayercolor{$featurecolor{$objectline[1]}});		
				$occupationleft{$objectline[6]} = $objectstart;
				$occupationright{$objectline[6]} = $objectend;
			}
			if($featurenamelength)
			{
				$featurenamewrap->set(text => $featurename{$objectline[6]});
				$featurenamewrap->draw($objectend,$imageheader+$seqnumber*$unitseqheight+$tracksize+$barheight+$goodlevel*($featuresize+1)+1);
				$objectend += $featurenamelength;
			}
