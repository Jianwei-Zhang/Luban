...
print "This scirpt is to collect the fastq files based on a given list of smrtcells.\n";
...
if (-f $smrtcells) # if provide a list of smrtcells, can be used for collecting data from original directories
{
	...
	while (<FILE>)
	{
		...
		`rsync $smrtcellLine[1]/Analysis_Results/*.fastq $targetDir/$smrtcellLine[0]/Analysis_Results`;
		$number++;
	}
	close FILE;
}
elsif(-d $smrtcells) # if provide a run directory, can be used for a collection data directory.
{
	...
	foreach my $cell (sort @cells)
	{
		...
		`rsync $smrtcells/$cell/Analysis_Results/*.fastq $targetDir/$cell/Analysis_Results`;
		$number++;
	}
}
else
